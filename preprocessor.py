...
class StackTrace:
    ...
    def summary(self) -> str:
        ...
        if len(self.lines) <= 4:
            return self.text
        return "\n".join(self.lines[:2] + ["  ... ({} frames omitted)".format(len(self.lines) - 4)] + self.lines[-2:])
...
class PreprocessorResult:
    ...
    def to_summary(self, max_dedup_lines: int = 50) -> str:
        ...
        parts = []
        parts.append(f"## Log Preprocessing Summary")
        parts.append(f"- **Total lines**: {self.total_lines}")
        parts.append(f"- **Unique lines**: {self.unique_lines} ({self.reduction_pct}% reduction)")
        parts.append(f"- **Noise removed**: {self.noise_lines_removed}")
        if self.first_timestamp and self.last_timestamp:
            parts.append(f"- **Time range**: {self.first_timestamp} → {self.last_timestamp}")
        ...
        high_freq = [(l, c, s) for l, c, s in self.deduplicated
                     if s not in (Severity.ERROR, Severity.WARNING, Severity.FATAL) and c >= 5]
        if high_freq:
...
def _normalize_for_dedup(line: str) -> str:
    ...
    # Normalize pure numeric sequences (ports, PIDs, etc.) — be conservative
